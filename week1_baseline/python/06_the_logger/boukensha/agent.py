...
class Agent:
    ...
    6. Logs all activity to session-based JSONL files
    ...
    def _handle_tool_calls(self, content: List[Dict[str, Any]], response: Dict[str, Any]) -> None:
        ...
        for block in tool_calls:
            ...
            try:
                ...
                result = f"ERROR: {e.__class__.__name__}: {str(e)}"
    ...
    def _normalized_usage(self, response: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        """
        # Try standard usage fields first
        if "usage" in response:
            return response["usage"]
        if "usageMetadata" in response:
            return response["usageMetadata"]

        # Fallback to Ollama-style fields
        usage = {}
