...
def repl(
    *,
    ...
    # Register MCP servers from config and get summary
    ...
    try:
        Repl(
            ...
            version=__version__,
            api_key=api_key,
            servers=servers_summary,  # NEW: pass server summary
        ).start()
...
def _register_mcp_servers(registry, cfg: Config) -> Dict[str, int]:
    ...
    for name, entry in servers.items():
        ...
        try:
            ...
            if is_required:
                raise Exception(
                    f"boukensha: MCP server '{name}' failed to start: {e}\n"
                    f"       Check that '{entry['command']}' is installed and working."
                )
            else:
                # Warn about optional server failure and continue
                import sys
                if not is_quiet():
                    print(
